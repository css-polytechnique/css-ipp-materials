...
author: 
    - name: E. Bonutti d'Agostini
      email: emma.bonutti@sciencespo.fr
      role: "Author"
    - name: É. Schultz
...
Nevertheless, sociologists have developed their own forms to complete similar tasks. For instance, in the 1990s, Franzosi (1989) had conceptualised a subject-action-object coding scheme to extract information from newspapers. In the 2010s, some research projects used syntax-based parsers to speed up the analysis. For instance, Goldenstein and Poschmann retrieved subject-verb-object triplets from business to study corporate responsibility discourse. However, syntax-based parsers need rigid assumptions regarding the information parsed, making it difficult to observe shifts or boundaries of established categories. For more examples, [read the full article (p4-5)](https://osf.io/wjvfq_v1). Some

One of the greatest advantages of using genAI for IE tasks is its flexibility. Until now, researchers had developed highly specific tools that did not generalise to new contexts. Prompt-based IE mitigates this limit; however, to correctly use such tools, one needs to rigorously define the information they want to extract.

# The limits of using generative AI for IE 

One of the major pitfalls of using genAI for IE is the non-randomness of errors. Indeed, generative LLMs are famously known for learning patterns learned from their training data and repeating their biases. Working with potentially biased predictions can be seriously hazardous for the downstream analysis.

Furthermore, using genAI for your research is not devoid of problems. Still today, decoder models’ performance is very task-dependent (Ollion et al., 2023). Also, relying on proprietary models hinders project reproducibility and results stability (Barrie et al., 2025). Finally, environmental (Luccioni, 2024), privacy and copyright issues need to be considered seriously before using these tools. This is especially true if you are sending content to a third party, using an API or an interface. 

# Information Extraction in practice

In this tutorial we demonstrate how to use the `openai` Python library and we analyse obituaries to retrieve the gender of a deceased person and the educational institution they attended.

## Data

We use a sample of 300 obituaries and metadata from the New York Times. For confidentiality reasons, the sample has been altered; the sample is synthetic. The goal is to retrieve the gender of the deceased person as well as the educational institution attended. 
...
The dataset contains many columns listed below:
...
We will only use the column `"date_death"` and `"text"` to extract information from and `"gender"`, `"educ_inst"` as a ground truth.
...
Now to compare and evaluate the performace: 
...
- When the model truly makes a mistake, for instance it retrieves "a small film school in New York City" when it is absent from the golden truth.
- When the model finds the right answer but spelled differently, for instance it returns "Brown University" where the golden truth is "Brown".
...
- human as a judge: check where the institutions do not match and a judge can arbitrate if this is a spelling mistake or a true mistake.
- LLM as a judge: ask an LLM to compare the institutions and judge if it is a spelling mistake or a true mistake.

We will setup a formating strategy and use the levenshtein distance to allow for minor spelling mistakes. Finally a human will be make the final validation.
...
Finally, we can save the mismatching rows as an excel file, validate the results by hand and remerge the results: 
...
In this tutorial, we presented how to leverage generative LLMs to rapidly retrieve information in a text. This technique is particularly interesting if you possess large amounts of unstructured textual data and want to retrieve specific pieces of information. Using genAI does not relieve researchers from the conceptualisation work of defining the information to extract and the relevance for their work.

We covered how to make API calls with the `openAI` library and demonstrated with two cases from (synthetic) obituaries from the NYT: retrieving the gender of a deceased person and the educational institution they attended. The first task was fairly easy with good accuracies whereas the second task was trickier in itself, but also trickier to evaluate if the retrieved information is correct. To arbitrate difficult cases, we covered several solutions based on automatic formatting and the human-as-a-judge method. 

Now, if you want to use IE in your research project, you might want to consider trying different models to find one that best suits your needs. The paper highlights that the llama 70B perfectly extracts explicit information such as the gender, the age, or the cause of death, but struggles with more complex information such as the education level or the origin.<br/> 

Another important aspect that needs reflection is the crafting of the prompt. Some techniques, like chain-of-thoughts, can significantly improve the accuracy of the IE. <br/>
